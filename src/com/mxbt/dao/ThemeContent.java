...
public class ThemeContent {
	...
	public List<IndexBean> selectAllArticles(int Sid) {
		List<IndexBean> mList = new ArrayList<IndexBean>();
		IndexBean bean = null;
		...
		try {
			/*String sql="SELECT Sid FROM SUBJECT WHERE Scontent="+Scontent+"";
			...
			if(resultSet.next()){
				...
				while (resultSet1.next()) {
					...
					if(resultSet2.next()){
						...
						bean.setDateTime(dateTime);// 3,datetime
						/* 依据文章分类id，找到具体的分类内容——》写到Bean中 */
						String sql3 = "SELECT Content FROM kind WHERE Kid=" + kindid;
						...
						if(resultSet3.next()){
							...
							String sql4 = "SELECT Ipath FROM image WHERE Iid=" + coverid;
							...
							if(resultSet4.next()){
								...
								if(resultSet5.next()){
									...
									String sql6 = "SELECT Tpath FROM TEXT WHERE Tid=" + txtid;
									...
									if(resultSet6.next()){
										...
										if(resultSet7.next()){
											...
											nickName = resultSet7.getString("Unickname");
											bean.setNickName(nickName);// 7，nickname

											/* 依据头像id，找到用户对应的头像地址，——》写到Bean中 */

											String sql8 = "SELECT Ipath FROM image WHERE Iid=" + headid;
											statement8 = connection.prepareStatement(sql8);
											resultSet8 = statement8.executeQuery();
											if(resultSet8.next()){
												headImg = resultSet8.getString("Ipath");
