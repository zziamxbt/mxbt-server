...
public class ArticleComment_excute {
	...
	public List<ArticleCommentBean> data_select(int Aid){
		list=new ArrayList<ArticleCommentBean>();
		ArticleCommentBean bean=null;
		try {
			connection=C3P0Utils.getConnection();
			state=connection.prepareStatement("SELECT * FROM article_comment WHERE ACOaid="+Aid+" ORDER BY ACOcreatetime DESC");
			result=state.executeQuery();
     		while(result.next()){
				flag=true;
				 bean=new ArticleCommentBean();
				 Uid=result.getInt("ACOuid");
				 bean.setUid(Uid);
...
//				
//				 }
				 bean.setContent(result.getString("ACOcontent"));
				 bean.setCreatetime(result.getString("ACOcreatetime"));
				 state2=connection.prepareStatement("select * from user where Uid="+result.getInt("ACOuid"));
	...
	public void data_insert(int Aid,int Uid,String createtime,String content){
		try {
			connection=C3P0Utils.getConnection();
			state=connection.prepareStatement("insert into article_comment (ACOaid,ACOuid,ACOcontent,ACOcreatetime) values(?,?,?,?)");
			state.setInt(1, Aid);
			state.setInt(2, Uid);
	...
	public List<ArticleCommentBean> reply_select(int Aid){
		list2=new ArrayList<ArticleCommentBean>();
		ArticleCommentBean article=null;
		try {
			connection=C3P0Utils.getConnection();
			state=connection.prepareStatement("select * from article_reply where AREaid="+Aid+" order by AREcreatetime DESC");
...
//				state=connection.prepareStatement("select * from article_reply where AREreplyed="+data.get(j)+" order by AREcreatetime DESC");
				...
				while(result.next()){
					...
					article.setUid(result.getInt("AREreplyer"));
					article.setContent(result.getString("AREcontent"));
					article.setCreatetime(result.getString("AREcreatetime"));
					state2=connection.prepareStatement("select * from user where Uid="+result.getInt("AREreplyer"));
					result2=state2.executeQuery();
					while(result2.next()){
	...
	public void reply_insert(int Aid,int Uid,int replyed,String content,String createtime){
		String zifu="";
		try {
			connection=C3P0Utils.getConnection();
			...
			state=connection.prepareStatement("insert into article_reply (AREaid,AREreplyer,AREreplyed,AREcontent,AREcreatetime) values(?,?,?,?,?)");
			state.setInt(1, Aid);
			state.setInt(2, Uid);
