...
public class vote_excute {
	...
	public void updatenum(int Uid,String Cid) {
		...
		try {
			connection = C3P0Utils.getConnection();
			state = connection
					.prepareStatement("update andwrite set AWvote=AWvote+1 where AWauthor="
							+ Uid + " and AWcid=" + x);
			state.executeUpdate();
	...
	public void updatenum2(int Uid,String Cid) {
		x=Integer.parseInt(Cid);
		try {
			connection = C3P0Utils.getConnection();
			state = connection
